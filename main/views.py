...
def SearchView(request):
	if 'q' in request.GET:
		q = request.GET['q']
		menu = Menu.objects.filter(title__icontains=q)

	else:
		menu = False

	return render(request, "main/search.html", {"menu": menu})
...
def CheckoutView(request):
	...
	if request.method == 'GET':
		...
		order = {
			...
			'x_cor': user_data['x_cor'],
			'y_cor': user_data['y_cor'],
